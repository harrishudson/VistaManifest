<?php
 ...
 $nonce = page_top("Radar Data - Unidata NEXRAD Composites (GINI) - dhr/1km", "no");
?>
...
import { radar_reflectivity1_stops } 
 from '../../common/map_styles.js'

// Primary Map metadata
const gMAP_METADATA = {
 "proxy": "UCAR_thredds_proxy.php?url=",
 "day_endpoint_prefix": "/catalog/nexrad/composite/gini/dhr/1km",
 "subsetting_endpoint_prefix": "/ncss/grid/nexrad/composite/gini/dhr/1km",
 "subsetting_query_string_suffix": 
  "&addLatLon=true&accept=netcdf&format=netcdf3",
 "variable": 'Reflectivity',
 "map_title": 
  "Radar Data - Unidata NEXRAD Composites (GINI) - dhr/1km",
 "author_comment": 
  "Click to view Radar Value.",
 "related_links": 
  [{"label": "THREDDS Endpoint",
    "href": "https://thredds.ucar.edu/thredds/catalog/nexrad/composite/gini/dhr/1km/catalog.html"}],
 "map_attribution": 'Data &copy; UCAR',
 "map_type": 'grid',
 "layer_starting_opacity": 0.85,
 "cell_color_stops": radar_reflectivity1_stops,
 "cell_omit_value": function(val) {
  if (((!val) && (val != 0)) || (!isFinite(val)) || (val < 0))
   return true
  return false
 },
 "idealCellSize": 3  // Higher resolution okay for non animated layer
}
...
function opacityCell(cellData) {
 return cfu.steppedOpacity(cellData.value, gMAP_METADATA['cell_color_stops'])
}
...
function fetch_catalog_index() {
 if (!gDOCUMENT_HIDDEN) {
  let latest_endpoint = gMAP_METADATA["day_endpoint_prefix"] + '/catalog.xml'
  ...
  fetch(endpoint_url)
   ...
   .then(str => process_latest_day_index(str))
   .catch(error => { dequeue_throbber(); status_msg(error) })
 }
}

function process_latest_day_index(data) {
 if (!data) {
  dequeue_throbber()
 ...
 check_latest_day(TDS)
}

function check_latest_day(TDS) {
 ...
 fetch_latest_minute_index(latest_day)
}

function fetch_latest_minute_index(latest_day) {
 let latest_endpoint = gMAP_METADATA["day_endpoint_prefix"] + 
                       '/' +
                       latest_day +
                       '/catalog.xml'
 let proxy = gMAP_METADATA["proxy"]
 ...
 fetch(endpoint_url)
  .then(response => response.text())
  .then(str => process_latest_minute_index(str))
  .catch(error => { dequeue_throbber(); status_msg(error) })
}

function process_latest_minute_index(data) {
 if (!data) {
  dequeue_throbber()
  status_msg('No Data Found.')
  return
 ...
 var xmlDoc = parser.parseFromString(data,"text/xml")
 var TDS = new TDSCatalogParser(xmlDoc)
 check_latest_minute(TDS)
}

function check_latest_minute(TDS) {
 ...
 let minutes_rev_sorted = minutes.sort(function(a,b) { return (b.name - a.name) })
...
function redraw_map_layer() {
 ...
 let HorizStride = gNETCDF_TDS.getHorizStride(image_size, queryBounds, gMAP_METADATA['idealCellSize'])

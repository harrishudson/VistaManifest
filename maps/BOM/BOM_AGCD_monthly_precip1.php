<?php
 ...
 $nonce = page_top("BOM Historical Monthly Precipitation Totals", "no");
?>
...
import { createColorLegendImg, tile_providers, getOneMonthPrior } 
 from '../../common/map_helpers.js'
import { SphericalProjection }
 from '../../common/projection_helpers.js'
import { monthly_precipitation_totals_mm_stops } 
 from '../../common/map_styles.js'

// Primary Map metadata
const gMAP_METADATA = {
 "proxy": "BOM_thredds_proxy.php?url=",
 "catalog_endpoint": "/catalog/agcd/precip/total/r005/01month/",
 "subsetting_endpoint_prefix": "/ncss/grid/agcd/precip/total/r005/01month/",
 "subsetting_query_string_suffix": "&addLatLon=true&accept=netcdf&format=netcdf3",
 "variable": 'precip',
 "map_title": 'BOM Historical Monthly Precipitation Totals',
 "map_attribution": 'BOM',
 "map_type": 'grid',
 "author_comment": 
  "Select a Month to view Monthly Precipitation Totals.",
 "related_links": 
  [{"label": "THREDDS Endpoint",
    "href": "http://opendap.bom.gov.au/thredds/catalog/agcd/precip/total/r005/01month/catalog.html"}],
 "layer_starting_opacity": 0.5,
 "cell_color_stops": monthly_precipitation_totals_mm_stops,
...
function populate_dates(TDS) {
 ...
 let begin_month = yy_start.toString()+'-01'
 let end_month = yy_end.toString()+'-12'

 const monthPrior = getOneMonthPrior(new Date())
 const yyyy = monthPrior.getFullYear();
 const mm = String(monthPrior.getMonth() + 1).padStart(2, '0'); // Months are 0-indexed
 const formattedDate = `${yyyy}-${mm}`;

 let date_val = formattedDate 

 let d = document.getElementById('chosen_month')
 d.min = begin_month
 d.max = end_month
 d.value = date_val 
 d.disabled = false
...
function chosen_month_change(e) {
 fetch_year_metadata()
}

function fetch_year_metadata() {
 let d = document.getElementById('chosen_month').value
 let yy = d.substring(0,4)
 let mm = d.substring(5,7)

 let endpoint = 
  gMAP_METADATA["catalog_endpoint"] +
  yy

 let year_catalog_endpoint = endpoint + '/catalog.xml'

 let proxy = gMAP_METADATA["proxy"]
 let year_catalog_url = proxy + encodeURIComponent(year_catalog_endpoint)

 queue_throbber()

 fetch(year_catalog_url)
  .then(response => response.text())
  .then(str => process_year_metadata(str, yy, mm))
  .catch(error => { 
   dequeue_throbber()
   remove_overlay()
   clear_render()
...
function process_year_metadata(data, yy, mm) {
 dequeue_throbber()
 ...
 let parser = new DOMParser()
 var xmlDoc = parser.parseFromString(data,"text/xml")
 let year_tds = new TDSCatalogParser(xmlDoc)

 let datasets = year_tds.catalog_dataset_datasets

 let pattern = `_${yy}${mm}`
 let theFiles = 
  datasets.filter((d) => { return (d.name.indexOf(pattern) > -1) })
 ...
 let month_netcdf_endpoint = endpoint + '/dataset.xml'

 let proxy = gMAP_METADATA["proxy"]
 let month_netcdf_url = proxy + encodeURIComponent(month_netcdf_endpoint)

 queue_throbber()

 fetch(month_netcdf_url)
  .then(response => response.text())
...
function map_startup() {
 ...
 document.getElementById('chosen_month').addEventListener('change', chosen_month_change)
 document.getElementById('chosen_month').addEventListener('click',
  function(e) {
   e.stopPropagation()
   return false 
  })
...
<div class="full_screen">
 <div id="map" style="overflow:hidden; width:100%; height:100%;"> 
  <div class="map_headings">
   ...
   <input id="chosen_month" type="month" disabled="disabled">
  </div>

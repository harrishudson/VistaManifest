<?php
 ...
 $nonce = page_top('cpc_global_precip Daily total of precipitation', 'yes');
?>
...
import { mollweideProjection, inverseMollweideProjection, 
         isMollweidePointInProjection } 
 from '../../common/projection_helpers.js'
import { daily_precipitation_totals_mm_stops } from '../../common/map_styles.js'

// Primary Map metadata
const gMAP_METADATA = {
 "proxy": "PSL_thredds_proxy.php?url=",
 "catalog_endpoint": 
  "/catalog/Datasets/cpc_global_precip/catalog.xml",
 "subsetting_endpoint_prefix": 
  "/ncss/grid/Datasets/cpc_global_precip",
 "subsetting_query_string_suffix": 
  "&addLatLon=true&accept=netcdf&format=netcdf3",
 "variable": "precip",
 "author_comment": 
  "Select a day to display Daily Precipitation.",
 "related_links": 
  [{"label":"THREDDS Endpoint",
    "href":"https://psl.noaa.gov/thredds/catalog/Datasets/cpc_global_precip/catalog.html"}],
...
function populate_dates(TDS) {
 ...
 let stripped_array = datasets.map(x => x.name.replaceAll('precip.','').replaceAll('.nc',''))
 let yy_string_array = stripped_array.filter(function(x) { return isFinite(x) }) 
 let yy_array = yy_string_array.map(function(x) { return parseInt(x) }) 
...
<div class="indent">
 <h4>Dataset: cpc_global_precip. Daily Precipitation.</h4>
 <p>
  <dl>
   <dt>Select Day</dt>
   <dd><input id="chosen_day" type="date" disabled="true"/></dd>
   <dt>Controls</dt>
   <dd>

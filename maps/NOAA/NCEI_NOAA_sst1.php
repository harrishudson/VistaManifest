<?php
 ...
 $nonce = page_top('OISST: Optimum Interpolation Sea Surface Temperatures', 'yes');
?>
...
const gMAP_METADATA = {
 "catalog_endpoint": 
  "https://www.ncei.noaa.gov/thredds/catalog/OisstBase/NetCDF/V2.1/AVHRR/catalog.xml",
 "subsetting_endpoint_prefix": 
  "https://www.ncei.noaa.gov/thredds/ncss/OisstBase/NetCDF/V2.1/AVHRR/",
 ...
 "related_links": 
  [{"label":"THREDDS Endpoint",
    "href":"https://www.ncei.noaa.gov/thredds/catalog/OisstBase/NetCDF/V2.1/AVHRR/catalog.html"}],
...
function populate_dates(TDS) {
 ...
 let yymm_array = dataset_refs.map(x => x.title)
 yymm_array.sort((a, b) => a - b);
 let yymm_start = yymm_array[0]
 let yymm_end = yymm_array[yymm_array.length - 1]

 const begin = extractYearAndMonth(yymm_start);
 let begin_day = getFirstDayOfMonth(begin.year, begin.month)

 const end = extractYearAndMonth(yymm_end);
 let end_day = getLastDayOfMonth(end.year, end.month)

 //let cur = end.year.toString()+'-'+pad(end.month).toString()+'-'+pad(end_day).toString()
 //let date_val = getOneMonthPrior(new Date(cur))
 let date_val = getOneMonthPrior(new Date())

 let d = document.getElementById('chosen_day')
 d.min = begin.year.toString()+'-'+pad(begin.month).toString()+'-'+pad(begin_day).toString()
 d.max = end.year.toString()+'-'+pad(end.month).toString()+'-'+pad(end_day).toString()
 d.value = formatDateToYYYYMMDD(date_val) 
 d.disabled = false

 gDATASET_TIME_MESSAGE = 
  'Dates available; '+
  begin.year.toString()+ ' to ' +
  end.year.toString()
...
function fetch_date_metadata() {
 ...
 gCHOSEN_DAY = d
 let chosen_day = d.replaceAll('-','')
 let chosen_month = chosen_day.substring(0,6)
...
<div class="indent">
 <h4>OISST: Optimum Interpolation Sea Surface Temperatures</h4>
 <p>
  <dl>
   <dt>Select Day</dt>

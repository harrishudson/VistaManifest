<?php
 ...
 $nonce = page_top("Daymet V4 R1 Precipitation for Continental North America", "no");
?>
...
const gMAP_METADATA = {
 ...
 "variable": 'prcp',
 "map_title": 
  "Daymet V4 R1 Precipitation for Continental North America",
 "author_comment": 
  "Select a Day to view Daily Precipitation Totals.",
 "related_links": 
...
function populate_dates(TDS) {
 ...
 let reg = new RegExp('precipitation for Continental North America')
 let rain_datasets =
  datasets.filter(function(obj) {return reg.test(obj.name)});

 let yy_array = 
  rain_datasets.map(function(x) { return  x.name
    .replaceAll('Daymet V4 R1 precipitation for Continental North America (','')
    .replaceAll(')','')})

<?php
 ...
 $nonce = page_top("Daymet V4 R1 Daily Maximum Temperature for Continental North America", "no");
?>
...
const gMAP_METADATA = {
 ...
 "variable": 'tmax',
 "map_title": 
  "Daymet V4 R1 Daily Maximum Temperature for Continental North America",
 "author_comment": 
  "Select a Day to view Daily Maximum Temperatures.",
 "related_links": 
...
function populate_dates(TDS) {
 ...
 let reg = new RegExp('daily maximum temperature for Continental North America')
 let rain_datasets =
  datasets.filter(function(obj) {return reg.test(obj.name)});

 let yy_array = 
  rain_datasets.map(function(x) { return  x.name
    .replaceAll('Daymet V4 R1 daily maximum temperature for Continental North America (','')
    .replaceAll(')','')})

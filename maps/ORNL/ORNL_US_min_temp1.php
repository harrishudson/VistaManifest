<?php
 ...
 $nonce = page_top("Daymet V4 R1 Daily Minimum Temperature for Continental North America", "no");
?>
...
const gMAP_METADATA = {
 "proxy": "ORNL_thredds_proxy.php?url=",
 "catalog_endpoint": 
  "/catalog/ornldaac/2129/catalog.xml",
 "subsetting_endpoint_prefix": 
  "/ncss/grid/ornldaac/2129/",
 "subsetting_query_string_suffix": 
  "&addLatLon=true&accept=netcdf&format=netcdf3",
 "variable": 'tmin',
 "map_title": 
  "Daymet V4 R1 Daily Minimum Temperature for Continental North America",
 "author_comment": 
  "Select a Day to view Daily Minimum Temperatures.",
 "related_links": 
  [{"label": "THREDDS Endpoint",
    "href": "https://thredds.daac.ornl.gov/thredds/catalog/ornldaac/2129/catalog.html"},
   {"label": "THREDDS README",
    "href": "https://thredds.daac.ornl.gov/thredds/catalog/ornldaac/2129/catalog.html?dataset=2129"}],
 "map_attribution": 'Data &copy; ORNL',
 "map_type": 'grid',
...
function populate_dates(TDS) {
 ...
 let reg = new RegExp('daily minimum temperature for Continental North America')
 let rain_datasets =
  datasets.filter(function(obj) {return reg.test(obj.name)});

 let yy_array = 
  rain_datasets.map(function(x) { return  x.name
    .replaceAll('Daymet V4 R1 daily minimum temperature for Continental North America (','')
    .replaceAll(')','')})
 ...
 let date_val = end_day  // getOneMonthPrior(new Date())

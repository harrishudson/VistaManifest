<?php
 ...
 $nonce = page_top('ANU Water and Landscape Dynamics - AU - Biodiversity', 'no');
?>
...
const gMAP_METADATA = {
 "proxy": null,
 "catalog_endpoint": 
  "https://thredds.nci.org.au/thredds/catalog/ub8/au/biodiversity/catalog.xml",
 "subsetting_endpoint_prefix": 
  "https://thredds.nci.org.au/thredds/ncss/grid/ub8/au/biodiversity/",
 "subsetting_query_string_suffix": 
  "&addLatLon=true&accept=netcdf&format=netcdf3",
 "map_title": 
  "ANU Water and Landscape Dynamics - AU - Biodiversity",
 "author_comment": 
  "Select a File then a Taxonomy.",
 "related_links": 
  [{"label":"THREDDS Endpoint",
    "href": "https://thredds.nci.org.au/thredds/catalog/ub8/au/biodiversity/catalog.html"}],
...
function fetch_catalog_index() {
 let sv = document.getElementById('select_taxonomy')
 sv.innerHTML = '<option value="none" selected="selected">Taxonomy</option>'
 sv.disabled = true
...
function populate_files(TDS) {
 let datasets = TDS.catalog_dataset_datasets
 let s = document.getElementById('select_file')
 s.innerHTML = '<option value="none" selected="selected">File</option>'

 datasets.forEach(optionData => {
  const option = document.createElement('option')
  option.value = optionData.name
  option.textContent = optionData.name.replaceAll('.nc','')
  s.appendChild(option)
  }) 
}

function select_file_change(e) {
 remove_overlay()
 clear_render()
 ...
 fetch(endpoint_url, {
  ...
  .then(response => response.text())
  .then(str => process_taxonomy_index(str, endpoint))
  .catch(error => { dequeue_throbber(); status_msg(error) })
}

function process_taxonomy_index(data, endpoint) {
 dequeue_throbber()
 ...
 populate_taxonomies(TDS, endpoint)
}

function populate_taxonomies(TDS, endpoint) {
 gNETCDF_TDS = TDS
 gNETCDF_SUBSET_ENDPOINT = endpoint

 let varList = TDS.getGridVariableList()

 let sv = document.getElementById('select_taxonomy')
 sv.innerHTML = '<option value="none" selected="selected">Taxonomy</option>'

 varList.forEach(optionData => {
  const option = document.createElement('option')
  option.value = optionData.name
  option.title = optionData['nestedAttributes']['comm_name']
  option.textContent = optionData['nestedAttributes']['comm_name'].substring(0,80)
...
function select_taxonomy_change(e) {
 gNETCDF_VAR = null

 let taxonomy = this.value

 if (taxonomy == 'none') 
  return

 gNETCDF_VAR = taxonomy
...
function redraw_map_layer() {
 ...
 var query_string = "?var="+encodeURIComponent(gNETCDF_VAR)
...
async function render_image() {
 let img = await CFR.draw2DbasicGrid(gNETCDF_VAR,
                                     null,
                                     SphericalProjection, 
                                     'url',
...
function map_startup() {
 ...
 document.getElementById('select_file').addEventListener('change', select_file_change)
 document.getElementById('select_file').addEventListener('click', 
  function(e) {
   e.stopPropagation()
   return false 
  })
 document.getElementById('select_taxonomy').addEventListener('change', select_taxonomy_change)
 document.getElementById('select_taxonomy').addEventListener('click', 
  function(e) {
   e.stopPropagation()
   return false 
  })
...
function map_tap(evt) {
 ...
 var x = null;
 try {
  x = CFR.getCellValue(gNETCDF_VAR,
                       null, 
                       evt.latlng.lng,
                       evt.latlng.lat,
                       null)
 ...
 <div id="map" style="overflow:hidden; width:100%; height:100%;"> 
  <div class="map_headings">
   ...
   <select id="select_file">
    <option value="none">File</option>
   </select>
   <select id="select_taxonomy" disabled="disabled" style="max-width: 240px;">
    <option value="none">Taxonomy</option>
   </select>
  </div>
